...
import org.iatoki.judgels.jerahmeel.forms.ChapterProblemAddForm;
import org.iatoki.judgels.jerahmeel.forms.ChapterProblemEditForm;
...
import org.iatoki.judgels.jerahmeel.views.html.chapter.problem.addChapterProblemView;
import org.iatoki.judgels.jerahmeel.views.html.chapter.problem.editChapterProblemView;
import org.iatoki.judgels.jerahmeel.views.html.chapter.problem.listChapterProblemsView;
import org.iatoki.judgels.jerahmeel.views.html.chapter.problem.viewProblemView;
...
public final class ChapterProblemController extends AbstractJudgelsController {
    ...
    public Result viewChapterProblem(long chapterId, long chapterProblemId) throws ChapterNotFoundException, ChapterProblemNotFoundException {
        ...
        if (ChapterProblemType.BUNDLE.equals(chapterProblem.getType())) {
            ...
            param.setPostSubmitUrl(routes.ChapterBundleSubmissionController.postSubmitProblem(chapter.getId(), chapterProblem.getProblemJid()).absoluteURL(request(), request().secure()));
            ...
            param.setStatementLanguage(StatementControllerUtils.getCurrentStatementLanguage());
            param.setSwitchStatementLanguageUrl(routes.TrainingProblemController.switchLanguage().absoluteURL(request(), request().secure()));
            param.setPostSubmitUrl(routes.ChapterProgrammingSubmissionController.postSubmitProblem(chapter.getId(), chapterProblem.getProblemJid()).absoluteURL(request(), request().secure()));
    ...
    private void appendBreadcrumbsLayout(LazyHtml content, Chapter chapter, InternalLink... lastLinks) {
        ...
        breadcrumbsBuilder.add(new InternalLink(Messages.get("chapter.problems"), routes.ChapterController.jumpToProblems(chapter.getId())));
